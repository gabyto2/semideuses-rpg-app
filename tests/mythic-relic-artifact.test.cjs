...
const dodona=db.getMythicItem('relic-fragmento-da-proa-falante');
assert(dodona&&dodona.page===59&&dodona.active&&dodona.active.name==='Conselho de Dodona','O lote 09 deve incluir o Fragmento da Proa Falante.');
const aquiles=db.getMythicItem('relic-coroa-funebre-de-aquiles');
assert(aquiles&&aquiles.active&&aquiles.active.action==='Reação'&&aquiles.usage&&aquiles.usage.scope==='day','A Coroa Fúnebre de Aquiles deve registrar Reação e uso diário.');
