...
let mortalForge=Forge.preview(character(),{baseItemId:'espada-longa',materialId:'aco',quality:'Padrão',propertyIds:[],forgeId:'standard'});let cyclopsForge=Forge.preview(character({heroType:'Ciclope',rules:{proficiency:3}}),{baseItemId:'espada-longa',materialId:'aco',quality:'Padrão',propertyIds:[],forgeId:'standard'});assert.equal(cyclopsForge.timeDays,mortalForge.timeDays/2,'Mãos de Forja deve reduzir o tempo pela metade.');assert.equal(cyclopsForge.testBonus,6,'Especialização em ferramentas de ferreiro deve aplicar duas vezes a Proficiência.');
