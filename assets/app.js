...
function stepView(){
  ...
  if(state.step===6){
    ...
    if(c.level<3&&origin.id!=='legado')return '<span class="step-icon">🔒</span><h3>Caminho ainda não liberado</h3><p>Esta escolha aparece no nível 3.</p>';
...
function sheet(){
  var c=state.editing;recalc();c=state.editing;var definition=primaryDefinition(c),origin=originByCharacter(c),pathText=c.divinePath||((origin.id==='ciclope'||origin.id==='mortal-vidente')?'Não utiliza':'Escolha pendente');
