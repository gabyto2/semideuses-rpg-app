...
function moduleView(){
  if(state.section==='jogador')return '<section class="hero"><span class="eyebrow">MÓDULO JOGADOR</span><h2>Jogador</h2><p>Criação, fichas, progressão e combate.</p></section><section class="module-list"><button class="module-row" data-open-characters><span class="icon">♙</span><span><strong>Personagens</strong><small>Criar, editar, evoluir e usar fichas.</small></span><span class="status ready">Ativo</span></button></section>';
...
function bind(){
  ...
  document.querySelectorAll('[data-new]').forEach(function(button){button.onclick=function(){state.editing=Model.create();state.step=0;state.screen='wizard';renderAndTop();};});
  document.querySelectorAll('[data-open-sheet]').forEach(function(button){button.onclick=function(){var character=Service.get(button.dataset.openSheet);if(character){state.editing=character;state.screen='sheet';sessionStorage.setItem('semideuses.currentCharacterId',character.id);renderAndTop();}};});
  document.querySelectorAll('[data-edit]').forEach(function(button){button.onclick=function(){var character=Service.get(button.dataset.edit);if(character){state.editing=character;state.step=0;state.screen='wizard';renderAndTop();}};});
...
window.SemideusesApp={getEditing:function(){return state.editing?clone(state.editing):null;},isEditingPersisted:function(){return !!(state.editing&&Service.get(state.editing.id));},applyAttributeCreation:applyAttributeCreation,refresh:function(){if(state.editing){var current=Service.get(state.editing.id);if(current)state.editing=current;}render();},openSheet:function(id){var character=Service.get(id);if(character){state.editing=character;state.screen='sheet';renderAndTop();}}};
