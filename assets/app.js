(function(){
  ...
  function emptyCharacter(){
    return {
      ...
      rules:{version:1,attributes:{FOR:10,DES:10,CON:10,INT:10,SAB:10,CAR:10},skills:[],features:[],inventory:[]}
    };
  }
  ...
  function homeView(){
    return '<section class="hero">'+
      ...
      '<p>O primeiro módulo funcional já está disponível. As fichas ficam salvas somente neste aparelho.</p>'+
    '</section>'+
    '<section class="dashboard-grid">'+
      '<button class="stat-card" data-open-characters><strong>'+state.characters.length+'</strong><span>Personagens</span></button>'+
      '<article class="stat-card"><strong>0</strong><span>Campanhas</span></article>'+
      '<article class="stat-card"><strong>2</strong><span>Livros-base</span></article>'+
    '</section>'+
    '<section class="grid">'+sections.slice(1).map(function(item){
      return '<button class="card action" data-go="'+item.id+'"><span class="card-icon">'+item.icon+'</span><strong>'+escapeHtml(item.label)+'</strong><small>Abrir módulo</small></button>';
    }).join('')+'</section>'+
    '<section class="panel"><span class="status ready">Etapa concluída</span><h3>Gerenciamento de personagens</h3><ul>'+ 
      '<li>Criar e editar fichas básicas</li><li>Duplicar e excluir personagens</li><li>Salvar automaticamente no navegador</li><li>Modelo preparado para regras automáticas</li></ul></section>';
  }
  ...
  function moduleView(section){
    ...
    return '<section class="hero"><span class="eyebrow">MÓDULO '+escapeHtml(section.label.toUpperCase())+'</span><h2>'+escapeHtml(section.label)+'</h2><p>'+description(section.id)+'</p></section>'+
      '<section class="module-list">'+list.map(function(item){
        return '<button class="module-row" data-module="'+escapeHtml(item.id)+'"><span class="icon">'+item.icon+'</span><span><strong>'+escapeHtml(item.title)+'</strong><small>'+escapeHtml(item.text)+'</small></span><span class="status '+(item.status==='Ativo'?'ready':'')+'">'+escapeHtml(item.status)+'</span></button>';
      }).join('')+'</section>'+
  ...
  function charactersView(){
    ...
    return '<section class="section-heading"><div><span class="eyebrow">MÓDULO JOGADOR</span><h2>Personagens</h2><p>Fichas salvas neste aparelho.</p></div><button class="primary" data-new-character>+ Nova ficha</button></section>'+
      (state.characters.length===0?
        '<section class="panel empty"><span class="large-icon">♙</span><h2>Nenhuma ficha criada</h2><p>Crie seu primeiro personagem para testar o armazenamento local.</p><button class="primary" data-new-character>Criar personagem</button></section>':
        '<section class="character-list">'+state.characters.map(characterCard).join('')+'</section>')+
  ...
  function characterCard(character){
    ...
    return '<article class="character-card">'+
      '<button class="character-main" data-edit-character="'+character.id+'">'+
        '<span class="avatar">'+escapeHtml(initial)+'</span><span><strong>'+escapeHtml(character.name||'Sem nome')+'</strong><small>Nível '+Number(character.level||1)+(character.affiliation?' · '+escapeHtml(character.affiliation):'')+'</small></span></button>'+
      '<div class="card-actions"><button data-duplicate-character="'+character.id+'">Duplicar</button><button class="danger" data-delete-character="'+character.id+'">Excluir</button></div></article>';
  }
  ...
  function characterEditorView(){
    ...
    return '<section class="section-heading"><div><span class="eyebrow">EDITOR DE FICHA</span><h2>'+escapeHtml(c.name||'Novo Herói')+'</h2><p>Campos básicos da estrutura oficial.</p></div><button class="secondary" data-cancel-edit>Voltar</button></section>'+
      '<form class="panel form-panel" id="character-form">'+
        '<div class="form-grid">'+
          field('name','Nome do herói',c.name,'text',true)+
          field('player','Jogador',c.player,'text',false)+
          field('level','Nível',c.level,'number',true)+
          field('origin','Origem / Raça',c.origin,'text',false)+
          field('background','Antecedente',c.background,'text',false)+
          field('affiliation','Filiação',c.affiliation,'text',false)+
          field('divinePath','Caminho Divino',c.divinePath,'text',false)+
        '</div>'+ 
        '<label class="full-field"><span>Notas</span><textarea name="notes" rows="7" placeholder="História, poderes, itens e observações...">'+escapeHtml(c.notes)+'</textarea></label>'+ 
        '<div class="prepared-block"><strong>Estrutura já reservada</strong><p>Atributos, perícias, habilidades, inventário e progressão já existem no modelo interno da ficha e serão ativados nas próximas etapas.</p></div>'+ 
        '<div class="editor-actions"><button type="button" class="secondary" data-cancel-edit>Cancelar</button><button type="submit" class="primary">Salvar ficha</button></div>'+ 
      '</form>';
  }

  function field(name,label,value,type,required){
    return '<label><span>'+escapeHtml(label)+'</span><input name="'+name+'" type="'+type+'" value="'+escapeHtml(value)+'" '+(type==='number'?'min="1" max="20" inputmode="numeric" ':'')+(required?'required ':'')+'/></label>';
  }
  ...
  function technicalNote(id){
    if(id==='jogador')return 'O módulo Personagens já está ativo. Progressão e combate serão conectados ao mesmo modelo de ficha.';
  ...
  function showMessage(message){
    ...
    render();
    window.setTimeout(function(){if(state.message===message){state.message='';render();}},2200);
  }

  function render(){
    var active=activeSection();
    var content=state.screen==='characters'?charactersView():(state.section==='inicio'?homeView():moduleView(active));
    var title=state.screen==='characters'?'Personagens':active.label;
    app.innerHTML='<div class="app-shell">'+
      '<header class="topbar"><div><span class="eyebrow">SEMIDEUSES RPG 3E</span><h1>'+escapeHtml(title)+'</h1></div><div class="brand-mark">S3</div></header>'+
      (state.message?'<button class="toast" data-dismiss-message>'+escapeHtml(state.message)+'</button>':'')+
  ...
  function bottomNav(){
    return '<nav class="bottom-nav" aria-label="Navegação principal">'+sections.map(function(item){
      return '<button data-go="'+item.id+'" class="'+(state.screen!=='characters'&&state.section===item.id?'active':'')+'"><span>'+item.icon+'</span><small>'+escapeHtml(item.label)+'</small></button>';
    }).join('')+'</nav>';
  }
  ...
  function bindEvents(){
    ...
    document.querySelectorAll('[data-new-character]').forEach(function(button){button.addEventListener('click',function(){state.screen='characters';state.editing=emptyCharacter();state.screen='edit';render();window.scrollTo(0,0);});});
  ...
  try{render();}
  catch(error){app.innerHTML='<main style="padding:24px;font-family:system-ui"><h1>Falha ao iniciar</h1><pre style="white-space:pre-wrap">'+escapeHtml(error&&error.message?error.message:error)+'</pre></main>';}
})();