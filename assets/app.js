...
function stepTitle(step){if(step!==3)return steps[step];var origin=originByCharacter(state.editing);return origin&&origin.requiresAffiliation?'Filiação':'Detalhes da origem';}
function wizardStepTitle(){return stepTitle(state.step);}
function choiceSearch(kind,label,placeholder){return '<label class="choice-search"><span>'+label+'</span><input type="search" data-choice-search="'+kind+'" placeholder="'+esc(placeholder)+'" autocomplete="off"></label><div class="choice-search-empty" data-choice-search-empty="'+kind+'" hidden>Nenhuma opção encontrada. Limpe a busca ou tente outro nome.</div>';}
function affiliationChoices(character){return choiceSearch('affiliation','Buscar Filiação','Ex.: Atena, guerra, natureza')+'<div class="choice-grid gods-grid" data-choice-list="affiliation">'+affiliationCatalog().map(function(affiliation){return '<button class="choice god-choice '+(character.affiliation===affiliation.name?'selected':'')+'" data-choice-option data-choice-label="'+esc(affiliation.name+' '+(affiliation.domain||'')+' '+(affiliation.profile||''))+'" data-aff="'+esc(affiliation.name)+'"><span class="choice-icon">'+esc(affiliation.icon||'✦')+'</span><strong>'+esc(affiliation.name)+'</strong><small>'+esc(affiliation.domain||affiliation.profile||'')+'</small></button>';}).join('')+'</div>';}
...
function firstSteps(){
  var c=state.characters[0],created=!!c,hasItem=state.characters.some(function(character){return (character.inventory||[]).length>0;}),hasCombat=state.characters.some(function(character){var session=character.session||{};return !!session.inCombat||(session.history||[]).some(function(entry){return entry.kind==='combat';});});
  if(created&&hasItem&&hasCombat)return '';
  function row(done,number,title,detail,action){return '<article class="'+(done?'done':'')+'"><b>'+(done?'✓':number)+'</b><span><strong>'+title+'</strong><small>'+detail+'</small></span>'+action+'</article>';}
  return '<section class="panel first-steps" data-first-steps><header><div><span class="eyebrow">PRIMEIROS PASSOS</span><h3>Prepare a primeira sessão</h3><p>Três ações curtas para confirmar que a ficha está pronta para jogar.</p></div><small>'+[created,hasItem,hasCombat].filter(Boolean).length+' / 3 concluídos</small></header><div class="first-step-list">'+
    row(created,1,'Criar uma ficha',created?'Ficha salva neste aparelho.':'Escolha Natureza, Filiação e nível da campanha.',created?'':'<button class="primary" data-new>Criar ficha</button>')+
    row(hasItem,2,'Adicionar o primeiro item',hasItem?'O item já aparece no Inventário.':created?'Abra a ficha; o atalho levará direto a Itens.':'Conclua primeiro a criação da ficha.',hasItem||!created?'':'<button class="secondary" data-open-sheet="'+esc(c.id)+'" data-first-step="items">Abrir Itens</button>')+
    row(hasCombat,3,'Testar o modo de combate',hasCombat?'O controle de rodadas já foi usado.':created?'Abra a ficha; o atalho levará aos controles de Combate.':'Conclua primeiro a criação da ficha.',hasCombat||!created?'':'<button class="secondary" data-open-sheet="'+esc(c.id)+'" data-first-step="combat">Abrir Combate</button>')+
  '</div></section>';
}
function moduleView(){
...
function listView(){
  refreshCharacters();
  return '<section class="section-heading"><div><span class="eyebrow">MÓDULO JOGADOR</span><h2>Personagens</h2><p>Fichas salvas neste aparelho.</p></div><button class="primary" data-new>Criar nova ficha</button></section>'+
    (state.characters.length?'<section class="character-list">'+state.characters.map(card).join('')+'</section>':'<section class="panel empty"><span class="large-icon">♙</span><h2>Nenhuma ficha neste aparelho</h2><p>Crie uma personagem nova ou use <strong>Importar sem apagar</strong> na área de segurança abaixo.</p><button class="primary" data-new>Criar primeira ficha</button></section>');
}
...
function wizard(){
  ...
  var previous=position.index?stepTitle(position.active[position.index-1]):'',next=position.index<position.active.length-1?stepTitle(position.active[position.index+1]):'';
  return '<section class="wizard-head"><button class="secondary compact-btn" data-cancel>Fechar criação</button><div><span class="eyebrow">CRIAÇÃO GUIADA</span><h2>'+wizardStepTitle()+'</h2><small>Passo '+number+' de '+total+'</small></div></section><div class="progress"><span style="width:'+(number/total*100)+'%"></span></div><section class="panel wizard-card">'+stepView()+'</section><div class="wizard-actions">'+(position.index?'<button class="secondary" data-prev>Voltar para '+esc(previous)+'</button>':'<span></span>')+(state.step===8?'<button class="primary" data-save>Salvar ficha e montar nível '+creationTargetLevel()+'</button>':'<button class="primary" data-next>Continuar para '+esc(next)+'</button>')+'</div>';
}
...
function stepView(){
  ...
  if(state.step===5)return '<h3>Antecedente</h3><p>Cada Antecedente aplica três proficiências, uma ferramenta, um Traço e um Vínculo.</p>'+choiceSearch('background','Buscar Antecedente','Ex.: atleta, investigação, sobrevivência')+'<div class="choice-list" data-choice-list="background">'+backgroundCatalog().map(function(background){return backgroundChoice(background,c.background===background.name);}).join('')+'</div>';
  if(state.step===6){
...
function bind(){
  ...
  document.querySelectorAll('[data-open-sheet]').forEach(function(button){button.onclick=function(){var character=Service.get(button.dataset.openSheet);if(character){state.section='jogador';state.editing=character;state.screen='sheet';sessionStorage.setItem('semideuses.currentCharacterId',character.id);if(button.dataset.firstStep)sessionStorage.setItem('semideuses.pendingSheetJump',button.dataset.firstStep);renderAndTop();}};});
  ...
  document.querySelectorAll('[data-choice-search]').forEach(function(input){input.oninput=function(){var kind=input.dataset.choiceSearch,term=String(input.value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim(),visible=0;document.querySelectorAll('[data-choice-list="'+kind+'"] [data-choice-option]').forEach(function(option){var haystack=String(option.dataset.choiceLabel||option.textContent||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase(),show=!term||haystack.indexOf(term)>=0;option.hidden=!show;if(show)visible+=1;});var empty=document.querySelector('[data-choice-search-empty="'+kind+'"]');if(empty)empty.hidden=visible!==0;};});
