(function(global){
  ...
  function payload(){return {format:FORMAT,edition:'3e',schemaVersion:2,exportedAt:new Date().toISOString(),campaign:Campaign.read(),encounters:Runtime.exportData(),calculator:Calculator.read()};}
  ...
  function restore(raw){var data=parse(raw);if(safety())safety().snapshot('antes de restaurar os dados do Mestre');Campaign.write(data.campaign);Runtime.restoreData(data.encounters);Calculator.write(data.calculator);if(safety())safety().recommendBackup({kind:'master-imported'});global.dispatchEvent(new CustomEvent('semideuses:master-backup-restored'));return {party:Campaign.read().party.length,sessions:Campaign.read().sessions.length,history:Runtime.history().length,savedEncounters:Calculator.read().savedEncounters.length};}
  function download(){var data=payload(),blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='semideuses-mestre-backup-'+new Date().toISOString().slice(0,10)+'.json';document.body.appendChild(link);link.click();link.remove();setTimeout(function(){URL.revokeObjectURL(url);},1000);if(safety())safety().recordExternalBackup('master-file',1);return data;}
  ...
  global.SemideusesMasterBackup={version:'master-backup-0.3.0',format:FORMAT,localKey:LOCAL_KEY,payload:payload,parse:parse,restore:restore,download:download,manualBackup:manualBackup,restoreManualBackup:restoreManualBackup,readFile:readFile};
})(window);
