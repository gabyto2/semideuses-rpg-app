(function(global){
  ...
  function restore(raw){var data=parse(raw);Campaign.write(data.campaign);Runtime.restoreData(data.encounters);Calculator.write(data.calculator);global.dispatchEvent(new CustomEvent('semideuses:master-backup-restored'));return {party:Campaign.read().party.length,sessions:Campaign.read().sessions.length,history:Runtime.history().length,savedEncounters:Calculator.read().savedEncounters.length};}
  ...
  global.SemideusesMasterBackup={version:'master-backup-0.2.0',format:FORMAT,localKey:LOCAL_KEY,payload:payload,parse:parse,restore:restore,download:download,manualBackup:manualBackup,restoreManualBackup:restoreManualBackup,readFile:readFile};
})(window);
